...
export function TopBar({ setAdd }) {
    ...
    return (
        <div className="flex justify-between bg-B1 border-2 border-black text-white italic">
            <div className="flex">
                <TopButtons1 onClick={() => {
                    console.log("lmao");
                    setAdd(add => !add);
                }} title={"Add"}></TopButtons1>
            </div>
...
function TopButtons1({ title, onClick }) {
    return <div className="border-2 border-black p-1 m-1 rounded-md bg-red-700 text-white hover:bg-red-300 hover:text-black cursor-pointer select-none" onClick={onClick}>{title}</div>;
}
function TopButtons2({ title, onClick }) {
    return <div className="border-2 border-black p-1 m-1 rounded-md bg-red-300 text-black cursor-pointer select-none" onClick={onClick}>{title}</div>;
}
